...
public class ShadowRenderPass {
    ...
    public static void loadShadowMap(DirectionalLight d) {
        ...
        for (int i = 0; i < NUMBER_CASCADES; i++) {
            ...
            for (OxyEntity e : SceneLayer.getInstance().allModelEntities) {
                ...
                shadowMapDepthShader.disable();

                OxyRenderer.renderMesh(mesh, shadowMapDepthShader);
            }
    ...
    public static Vector3f getShadowMapDirection(int index) {
        return new Vector3f(cascadedCamArr[index].directionalLight.getDirection()).mul(-1);
    }
