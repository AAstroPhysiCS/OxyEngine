...
import static OxyEngine.Core.Context.Renderer.Mesh.Vertex.MAX_BONE_INFLUENCE;
import static OxyEngine.Core.Context.Scene.SceneRuntime.entityContext;
import static OxyEngine.Utils.convertAIMatrixToJOMLMatrix;
...
public final class OpenGLMesh implements Disposable, EntityComponent {
    ...
    public OpenGLMesh(Pipeline pipeline, String path, MeshUsage usage) {
...
//        for (OxyMaterial material : other.materials) this.materials.add(OxyMaterial.create(material));
    ...
    public OpenGLMesh(Pipeline pipeline, MeshUsage usage) {
        this.pipeline = pipeline;
    ...
    public OpenGLMesh(Pipeline pipeline, int allocationSize, MeshUsage usage) {
        this.pipeline = pipeline;
        this.renderMode = this.pipeline.getRenderPass().getRenderMode();
