...
public abstract class FrameBuffer implements Disposable {
    ...
    public FrameBuffer getBlittedFrameBuffer() {
        return blitted;
    }

    public FrameBuffer createBlittingFrameBuffer(FrameBufferSpecification spec) {
    ...
    public static <T extends FrameBuffer> T create(Color clearColor, FrameBufferSpecification... specBuilders) {
        if (specBuilders.length == 0) throw new IllegalStateException("Specification not given!");
        if (Renderer.getCurrentTargetPlatform() == TargetPlatform.OpenGL) {
            try {
                var constructor = OpenGLFrameBuffer.class.getDeclaredConstructor(Color.class, specBuilders.getClass());
                constructor.setAccessible(true);
                return (T) constructor.newInstance(clearColor, specBuilders);
            } catch (Exception e) {
                e.getCause().printStackTrace();
            }
        }
    ...
    public abstract void flushDepthAttachment(int specIndex, int index);
}