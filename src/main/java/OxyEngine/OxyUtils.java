...
public interface OxyUtils {
    ...
    static Vector3f pxVec3ToJOMLVector3f(PxVec3 vec3) {
        return new Vector3f(vec3.getX(), vec3.getY(), vec3.getZ());
    }

    static PxVec3 vector3fToPxVec3(Vector3f vec3) {
        return new PxVec3(vec3.x(), vec3.y(), vec3.z());
    }

    static Quaternionf pxVec3ToJOMLQuaternionf(PxQuat quat) {
        return new Quaternionf(quat.getX(), quat.getY(), quat.getZ(), quat.getW());
    }

    static PxTransform matrix4fToPxTransform(Matrix4f transform) {
